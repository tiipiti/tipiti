...
- `ListItem`: `shopping_list`, `name` e `completed`.

`ListItem.name` aceita o texto que a pessoa escrever, inclusive quantidade e
unidade: "2 caixas de leite". Não há campos separados de quantidade, unidade,
produto ou categoria no fluxo inicial.
...
- No admin, a lista é um CRUD direto: nome e itens inline com texto e caixa de
  marcação. O administrador escolhe o dono ao criar a lista.
...
- Uma lista e seu item são criados com apenas nome; o item pode ser marcado
  como comprado com um booleano.
